...
Pan+zoom+drawing lines(only continuous loops allowed)
*/
...
const Canvas = ({ isDrawing,  isMoving, isDelete}) => {
    ...
    */
    ...
    //////// Utility fns. Type3: Convert grid coordinates+draw lines+ draw dots(draw circle) ////////
    
    const toScreenCoordinates = (x, y) => {
        //  Convert grid coordinates of a point (x, y) to the corresponding screen coordinates
        // (x, y) are grid coordinates of point
        return {
            x: (x * zoom) + pan.x,
            y: (y * zoom) + pan.y,
        };
    ...
    const drawDot = (ctx, x, y, isHovered) => {
        // Draws a dot at a specific point on the canvas
        const dotSize = 2 / zoom;
        
        ctx.beginPath();
        ctx.arc(x, y, dotSize, 0, 2 * Math.PI); // Draws an arc centered at (x, y) with radius dotSize and angle 2* Math.PI
    ...
    //////// Utility fns. Type4: Add Wall Clicking ////////
